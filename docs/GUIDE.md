...
> **Purpose:** This file defines HOW to write code for PurrfectReqs — patterns, conventions, and standard implementations. For WHAT to build, see `docs/SCOPE.md`. For security specs, see `docs/SECURITY.md`. For module structure rules, see `docs/ARCHITECTURE.md`. For frontend & UI standards, see `docs/FRONTEND.md`.
...
Frontend template structure, HTMX/Alpine usage, the component/macro architecture, design tokens, and the full UI Design Checklist now live in **`docs/FRONTEND.md`** — the single home for frontend guidance, so UI work loads without backend code patterns. The security-critical constraints those templates depend on remain governed by `docs/SECURITY.md` and `docs/TECH_STACK.md`.
...
The UI design checklist has moved to **`docs/FRONTEND.md`** → §7 "UI Design Checklist", alongside the rest of the frontend guidance (information architecture, the app shell, the template/component architecture, and design tokens). It was relocated so that frontend work can be loaded into context without pulling in backend code patterns, and vice versa.
