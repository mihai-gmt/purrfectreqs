# file: features/auth/20260407_basic_register_user.feature
# Version: 1.0
# Last Updated: 2026-04-05
# This feature contains the first iteration of the API endpoint for registering new users and its behaviour. No user interface components present in these requirements.
...
Feature: New user Account Registration
  ...
  So that I can access personalized features and use the service
    ...
    Scenario: Registration failure when email already exists
        ...
        When I register with the following details:
            | email | password | username | first_name | last_name |
            | john.doe1@example.com | Valid1234! | john.doe1 | John | Doe |
    ...
    @auth @new_user_registration @smoke @critical @fail
    Scenario: Registration failure when password does not meet the complexity requirements
